...
if name_anchor in text:
    ...
    # Fallback: inject next to another stable naming rule.
    fallback = "muscle_name은 생활언어의 자연스러운 '~하는 힘' 형태로 짧게 쓴다."
    if fallback not in text:
        raise SystemExit('v10.18.7 naming anchor missing')
    text = text.replace(fallback, fallback + " '억제/내성력/강화/조절 능력을 다루는 힘' 같은 임상적 명칭 대신 '불안할 때 잠깐 멈추는 힘', '관계 신호를 서두르지 않고 보는 힘', '내 에너지에 맞춰 사람과 다시 연결하는 힘'처럼 생활언어를 쓴다.", 1)

# 2) Tiny stage-1 gate using a stable schema anchor.
stable_step_anchor = "각 프로그램마다 steps를 정확히 3개 반환하되,"
step_rule = ("1단계 최소유효강도 규칙: 첫 단계는 사용자가 오늘 한 번 시도해도 '해냈어/조금/못했어'를 바로 누를 수 있어야 한다. 처음부터 '매일 3회', '2주간 3회', '10분 이상 버티기', '불안이 감소했는지 확인'처럼 횟수·기간·감정 결과를 성공조건으로 걸지 않는다. 관계 신호 PT라면 장면 1개에서 사실/해석을 한 번 나누면 1차 성공, 판정 지연 PT라면 2~3분만 미뤄도 1차 성공, 접촉 PT라면 편한 사람과 짧은 접촉 1회를 계획대로 마치고 전후 상태를 한 줄 기록하면 1차 성공으로 둔다. 횟수와 시간은 2단계 이후 실제 성공 기록에 맞춰 올린다. 성공기준은 감정이 사라졌는지가 아니라 이번 회차에 어떤 행동을 했는지로 쓴다. "
             + stable_step_anchor)
if stable_step_anchor not in text:
    raise SystemExit('v10.18.7 stable step anchor missing')
text = text.replace(stable_step_anchor, step_rule, 1)

# 3) Same rule at semantic merge stage.
merge_anchor = "성공기준은 '2주 후 불안 감소' 같은 장기 결과가 아니라 사용자가 이번 회차 또는 이번 주에 해냈어/조금/못했어를 판단할 수 있는 행동으로 쓴다. "
if merge_anchor in text:
    text = text.replace(merge_anchor, merge_anchor + "최종 PT 이름은 '억제/내성력/강화/조절 능력을 다루는 힘' 같은 임상적 명칭을 피하고 생활언어로 다시 이름 붙인다. 1단계 성공기준은 원칙적으로 한 장면·한 번의 시도로 판정 가능하게 하며, 반복 횟수나 긴 시간을 요구하는 목표는 2단계 이후로 미룬다. ", 1)

analyzer.write_text(text, encoding='utf-8')

s = shared.read_text(encoding='utf-8')
# 4) Hide verbose collector diagnostics from normal user-facing preview while keeping diagnostic code available.
s = s.replace(' + collectionDiagnosticSummary());', ');')
s = s.replace(' + collectionDiagnosticSummary() + "\\n\\n" + clip(full, 520)', ' + "\\n\\n" + clip(full, 520)')

# 5) Compact safety presence note in PT summary instead of a large block.
